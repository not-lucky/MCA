...
int main() {
  ...
  while (total_completed != num_processes) {
    for (int i = 0; i < num_processes; i++) {
      ...
      processes[to_work_ID].completion_time = current_time + processes[to_work_ID].burst_time;
      processes[to_work_ID].turnaround_time = processes[to_work_ID].completion_time - processes[to_work_ID].arrival_time;
      processes[to_work_ID].waiting_time = processes[to_work_ID].start_time - processes[to_work_ID].arrival_time;
      current_time += processes[to_work_ID].burst_time;
      processes[to_work_ID].completed = 1;
      total_completed++;
      if (processes[to_work_ID].completed == 0 && processes[to_work_ID].id != processes[i].id) {
        i--;
      }
      printf("%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n", processes[to_work_ID].id,
             processes[to_work_ID].arrival_time, processes[to_work_ID].burst_time, processes[to_work_ID].start_time,
             processes[to_work_ID].completion_time, processes[to_work_ID].turnaround_time,
